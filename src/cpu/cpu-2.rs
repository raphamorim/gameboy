// use crate::cpu::bit;
// use crate::cpu::data;
// use crate::cpu::reg::{Clock, Registers};
...
impl Cpu {
    ...
    pub fn fetchbyte(&mut self) -> u8 {
        let pc = self.memory.rb(self.reg.pc);
    ...
    fn pushstack(&mut self, value: u16) {
        self.reg.sp -= 2;
        self.memory.ww(self.reg.sp, value);
    }

    fn popstack(&mut self) -> u16 {
        let res = self.memory.rw(self.reg.sp);
        self.reg.sp += 2;
        res
    }

    pub fn fetchword(&mut self) -> u16 {
        let w = self.memory.rw(self.reg.pc);
        self.reg.pc += 2;
        w
    }
    ...
    fn call(&mut self) -> u32 {
        ...
        match opcode {
            ...
            0x02 => { self.memory.wb(self.reg.bc(), self.reg.a); 2 },
            ...
            0x08 => { let a = self.fetchword(); self.memory.ww(a, self.reg.sp); 5 },
            0x09 => { self.alu_add16(self.reg.bc()); 2 },
            0x0A => { self.reg.a = self.memory.rb(self.reg.bc()); 2 },
            ...
            0x12 => { self.memory.wb(self.reg.de(), self.reg.a); 2 },
            ...
            0x1A => { self.reg.a = self.memory.rb(self.reg.de()); 2 },
            ...
            0x22 => { self.memory.wb(self.reg.hli(), self.reg.a); 2 },
            ...
            0x2A => { self.reg.a = self.memory.rb(self.reg.hli()); 2 },
            ...
            0x32 => { self.memory.wb(self.reg.hld(), self.reg.a); 2 },
            0x33 => { self.reg.sp = self.reg.sp.wrapping_add(1); 2 },
            0x34 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_inc(v); self.memory.wb(a, v2); 3 },
            0x35 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_dec(v); self.memory.wb(a, v2); 3 },
            0x36 => { let v = self.fetchbyte(); self.memory.wb(self.reg.hl(), v); 3 },
            ...
            0x46 => { self.reg.b = self.memory.rb(self.reg.hl()); 2 },
            ...
            0x4E => { self.reg.c = self.memory.rb(self.reg.hl()); 2 },
            ...
            0x56 => { self.reg.d = self.memory.rb(self.reg.hl()); 2 },
            ...
            0x5E => { self.reg.e = self.memory.rb(self.reg.hl()); 2 },
            ...
            0x66 => { self.reg.h = self.memory.rb(self.reg.hl()); 2 },
            ...
            0x6D => { 1 },
            0x6E => { self.reg.l = self.memory.rb(self.reg.hl()); 2 },
            0x6F => { self.reg.l = self.reg.a; 1 },
            0x70 => { self.memory.wb(self.reg.hl(), self.reg.b); 2 },
            0x71 => { self.memory.wb(self.reg.hl(), self.reg.c); 2 },
            0x72 => { self.memory.wb(self.reg.hl(), self.reg.d); 2 },
            0x73 => { self.memory.wb(self.reg.hl(), self.reg.e); 2 },
            0x74 => { self.memory.wb(self.reg.hl(), self.reg.h); 2 },
            0x75 => { self.memory.wb(self.reg.hl(), self.reg.l); 2 },
            0x76 => { 
                self.halt = 1; 
                1 
            },
            0x77 => { self.memory.wb(self.reg.hl(), self.reg.a); 2 },
            ...
            0x7E => { self.reg.a = self.memory.rb(self.reg.hl()); 2 },
            0x7F => { 1 },
            ...
            0x86 => { let v = self.memory.rb(self.reg.hl()); self.alu_add(v, false); 2 },
            ...
            0x8E => { let v = self.memory.rb(self.reg.hl()); self.alu_add(v, true); 2 },
            ...
            0x96 => { let v = self.memory.rb(self.reg.hl()); self.alu_sub(v, false); 2 },
            ...
            0x9E => { let v = self.memory.rb(self.reg.hl()); self.alu_sub(v, true); 2 },
            ...
            0xA6 => { let v = self.memory.rb(self.reg.hl()); self.alu_and(v); 2 },
            ...
            0xAE => { let v = self.memory.rb(self.reg.hl()); self.alu_xor(v); 2 },
            ...
            0xB6 => { let v = self.memory.rb(self.reg.hl()); self.alu_or(v); 2 },
            ...
            0xBE => { let v = self.memory.rb(self.reg.hl()); self.alu_cp(v); 2 },
            ...
            0xE0 => { let a = 0xFF00 | self.fetchbyte() as u16; self.memory.wb(a, self.reg.a); 3 },
            0xE1 => { let v = self.popstack(); self.reg.sethl(v); 3 },
            0xE2 => { self.memory.wb(0xFF00 | self.reg.c as u16, self.reg.a); 2 },
            ...
            0xEA => { let a = self.fetchword(); self.memory.wb(a, self.reg.a); 4 },
            0xEE => { let v = self.fetchbyte(); self.alu_xor(v); 2 },
            0xEF => { self.pushstack(self.reg.pc); self.reg.pc = 0x28; 4 },
            0xF0 => { let a = 0xFF00 | self.fetchbyte() as u16; self.reg.a = self.memory.rb(a); 3 },
            0xF1 => { let v = self.popstack() & 0xFFF0; self.reg.setaf(v); 3 },
            0xF2 => { self.reg.a = self.memory.rb(0xFF00 | self.reg.c as u16); 2 },
            0xF3 => { 
                self.ime = 0;
                self.delay = 0;
            1 },
            ...
            0xFA => { let a = self.fetchword(); self.reg.a = self.memory.rb(a); 4 },
            0xFB => { 
                if self.delay == 2 || self.memory.rb(self.reg.pc) == 0x76 {
            self.delay = 1;
        } else {
            self.delay = 2;
        }
    ...
    fn call_cb(&mut self) -> u32 {
        ...
        match opcode {
            ...
            0x06 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_rlc(v); self.memory.wb(a, v2); 4 },
            ...
            0x0E => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_rrc(v); self.memory.wb(a, v2); 4 },
            ...
            0x16 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_rl(v); self.memory.wb(a, v2); 4 },
            ...
            0x1E => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_rr(v); self.memory.wb(a, v2); 4 },
            ...
            0x26 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_sla(v); self.memory.wb(a, v2); 4 },
            ...
            0x2E => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_sra(v); self.memory.wb(a, v2); 4 },
            ...
            0x36 => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_swap(v); self.memory.wb(a, v2); 4 },
            ...
            0x3E => { let a = self.reg.hl(); let v = self.memory.rb(a); let v2 = self.alu_srl(v); self.memory.wb(a, v2); 4 },
            ...
            0x46 => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 0); 3 },
            ...
            0x4E => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 1); 3 },
            ...
            0x56 => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 2); 3 },
            ...
            0x5E => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 3); 3 },
            ...
            0x66 => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 4); 3 },
            ...
            0x6E => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 5); 3 },
            ...
            0x76 => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 6); 3 },
            ...
            0x7E => { let v = self.memory.rb(self.reg.hl()); self.alu_bit(v, 7); 3 },
            ...
            0x86 => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 0); self.memory.wb(a, v); 4 },
            ...
            0x8E => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 1); self.memory.wb(a, v); 4 },
            ...
            0x96 => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 2); self.memory.wb(a, v); 4 },
            ...
            0x9E => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 3); self.memory.wb(a, v); 4 },
            ...
            0xA6 => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 4); self.memory.wb(a, v); 4 },
            ...
            0xAE => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 5); self.memory.wb(a, v); 4 },
            ...
            0xB6 => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 6); self.memory.wb(a, v); 4 },
            ...
            0xBE => { let a = self.reg.hl(); let v = self.memory.rb(a) & !(1 << 7); self.memory.wb(a, v); 4 },
            ...
            0xC6 => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 0); self.memory.wb(a, v); 4 },
            ...
            0xCE => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 1); self.memory.wb(a, v); 4 },
            ...
            0xD6 => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 2); self.memory.wb(a, v); 4 },
            ...
            0xDE => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 3); self.memory.wb(a, v); 4 },
            ...
            0xE6 => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 4); self.memory.wb(a, v); 4 },
            ...
            0xEE => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 5); self.memory.wb(a, v); 4 },
            ...
            0xF6 => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 6); self.memory.wb(a, v); 4 },
            ...
            0xFE => { let a = self.reg.hl(); let v = self.memory.rb(a) | (1 << 7); self.memory.wb(a, v); 4 },
