...
impl Cpu {
    ...
    fn ld_hlia(&mut self, _m: &mut Mmu) {
    ...
    fn program_counter_call(&mut self, op: u8, m: &mut Mmu) {
        println!("Instruction at {:#01x} | {:#06x} | {}", op, op, op);
        println!("{:?}", self._r);
        match op {
            0x00 => {
                println!("{:?} bateu zero", 0);
                self.nop();
                // self.xx();
            }
            0x01 => self.ld_bcnn(m),
