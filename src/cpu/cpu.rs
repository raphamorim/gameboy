...
impl Cpu {
    ...
    fn call(&mut self) -> u32 {
        ...
        match opcode {
            0x00 => { 1 },
            0x01 => { let v = self.get_word(); self.registers.setbc(v); 3 },
            0x02 => { self.memory.wb(self.registers.bc(), self.registers.a); 2 },
            ...
            0x06 => { self.registers.b = self.get_byte(); 2 },
            0x07 => { self.registers.a = self.alu_rlc(self.registers.a); self.registers.flag(Z, false); 1 },
            0x08 => { let a = self.get_word(); self.memory.ww(a, self.registers.sp); 5 },
            0x09 => { self.alu_add16(self.registers.bc()); 2 },
            0x0A => { self.registers.a = self.memory.rb(self.registers.bc()); 2 },
            ...
            0x0E => { self.registers.c = self.get_byte(); 2 },
            0x0F => { self.registers.a = self.alu_rrc(self.registers.a); self.registers.flag(Z, false); 1 },
            0x10 => { self.memory.switch_speed(); 1 }, // STOP
            0x11 => { let v = self.get_word(); self.registers.setde(v); 3 },
            0x12 => { self.memory.wb(self.registers.de(), self.registers.a); 2 },
            ...
            0x16 => { self.registers.d = self.get_byte(); 2 },
            0x17 => { self.registers.a = self.alu_rl(self.registers.a); self.registers.flag(Z, false); 1 },
            0x18 => { self.cpu_jr(); 3 },
            0x19 => { self.alu_add16(self.registers.de()); 2 },
            0x1A => { self.registers.a = self.memory.rb(self.registers.de()); 2 },
            ...
            0x1E => { self.registers.e = self.get_byte(); 2 },
            0x1F => { self.registers.a = self.alu_rr(self.registers.a); self.registers.flag(Z, false); 1 },
            0x20 => { if !self.registers.getflag(Z) { self.cpu_jr(); 3 } else { self.registers.pc += 1; 2 } },
            0x21 => { let v = self.get_word(); self.registers.sethl(v); 3 },
            0x22 => { self.memory.wb(self.registers.hli(), self.registers.a); 2 },
            0x23 => { let v = self.registers.hl().wrapping_add(1); self.registers.sethl(v); 2 },
            0x24 => { self.registers.h = self.alu_inc(self.registers.h); 1 },
            0x25 => { self.registers.h = self.alu_dec(self.registers.h); 1 },
            0x26 => { self.registers.h = self.get_byte(); 2 },
            0x27 => { self.alu_daa(); 1 },
            0x28 => { if self.registers.getflag(Z) { self.cpu_jr(); 3 } else { self.registers.pc += 1; 2  } },
            0x29 => { let v = self.registers.hl(); self.alu_add16(v); 2 },
            0x2A => { self.registers.a = self.memory.rb(self.registers.hli()); 2 },
            0x2B => { let v = self.registers.hl().wrapping_sub(1); self.registers.sethl(v); 2 },
            0x2C => { self.registers.l = self.alu_inc(self.registers.l); 1 },
            0x2D => { self.registers.l = self.alu_dec(self.registers.l); 1 },
            0x2E => { self.registers.l = self.get_byte(); 2 },
            0x2F => { self.registers.a = !self.registers.a; self.registers.flag(H, true); self.registers.flag(N, true); 1 },
            0x30 => { if !self.registers.getflag(C) { self.cpu_jr(); 3 } else { self.registers.pc += 1; 2 } },
            0x31 => { self.registers.sp = self.get_word(); 3 },
            0x32 => { self.memory.wb(self.registers.hld(), self.registers.a); 2 },
            0x33 => { self.registers.sp = self.registers.sp.wrapping_add(1); 2 },
            0x34 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_inc(v); self.memory.wb(a, v2); 3 },
            0x35 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_dec(v); self.memory.wb(a, v2); 3 },
            0x36 => { let v = self.get_byte(); self.memory.wb(self.registers.hl(), v); 3 },
            0x37 => { self.registers.flag(C, true); self.registers.flag(H, false); self.registers.flag(N, false); 1 },
            0x38 => { if self.registers.getflag(C) { self.cpu_jr(); 3 } else { self.registers.pc += 1; 2  } },
            0x39 => { self.alu_add16(self.registers.sp); 2 },
            0x3A => { self.registers.a = self.memory.rb(self.registers.hld()); 2 },
            ...
            0x3E => { self.registers.a = self.get_byte(); 2 },
            0x3F => { let v = !self.registers.getflag(C); self.registers.flag(C, v); self.registers.flag(H, false); self.registers.flag(N, false); 1 },
            0x40 => { 1 },
            ...
            0x46 => { self.registers.b = self.memory.rb(self.registers.hl()); 2 },
            ...
            0x4E => { self.registers.c = self.memory.rb(self.registers.hl()); 2 },
            ...
            0x56 => { self.registers.d = self.memory.rb(self.registers.hl()); 2 },
            ...
            0x5E => { self.registers.e = self.memory.rb(self.registers.hl()); 2 },
            ...
            0x66 => { self.registers.h = self.memory.rb(self.registers.hl()); 2 },
            ...
            0x6D => { 1 },
            0x6E => { self.registers.l = self.memory.rb(self.registers.hl()); 2 },
            0x6F => { self.registers.l = self.registers.a; 1 },
            0x70 => { self.memory.wb(self.registers.hl(), self.registers.b); 2 },
            0x71 => { self.memory.wb(self.registers.hl(), self.registers.c); 2 },
            0x72 => { self.memory.wb(self.registers.hl(), self.registers.d); 2 },
            0x73 => { self.memory.wb(self.registers.hl(), self.registers.e); 2 },
            0x74 => { self.memory.wb(self.registers.hl(), self.registers.h); 2 },
            0x75 => { self.memory.wb(self.registers.hl(), self.registers.l); 2 },
            0x76 => { self.halted = true; 1 },
            0x77 => { self.memory.wb(self.registers.hl(), self.registers.a); 2 },
            ...
            0x7E => { self.registers.a = self.memory.rb(self.registers.hl()); 2 },
            0x7F => { 1 },
            0x80 => { self.alu_add(self.registers.b, false); 1 },
            0x81 => { self.alu_add(self.registers.c, false); 1 },
            0x82 => { self.alu_add(self.registers.d, false); 1 },
            0x83 => { self.alu_add(self.registers.e, false); 1 },
            0x84 => { self.alu_add(self.registers.h, false); 1 },
            0x85 => { self.alu_add(self.registers.l, false); 1 },
            0x86 => { let v = self.memory.rb(self.registers.hl()); self.alu_add(v, false); 2 },
            0x87 => { self.alu_add(self.registers.a, false); 1 },
            0x88 => { self.alu_add(self.registers.b, true); 1 },
            0x89 => { self.alu_add(self.registers.c, true); 1 },
            0x8A => { self.alu_add(self.registers.d, true); 1 },
            0x8B => { self.alu_add(self.registers.e, true); 1 },
            0x8C => { self.alu_add(self.registers.h, true); 1 },
            0x8D => { self.alu_add(self.registers.l, true); 1 },
            0x8E => { let v = self.memory.rb(self.registers.hl()); self.alu_add(v, true); 2 },
            0x8F => { self.alu_add(self.registers.a, true); 1 },
            0x90 => { self.alu_sub(self.registers.b, false); 1 },
            0x91 => { self.alu_sub(self.registers.c, false); 1 },
            0x92 => { self.alu_sub(self.registers.d, false); 1 },
            0x93 => { self.alu_sub(self.registers.e, false); 1 },
            0x94 => { self.alu_sub(self.registers.h, false); 1 },
            0x95 => { self.alu_sub(self.registers.l, false); 1 },
            0x96 => { let v = self.memory.rb(self.registers.hl()); self.alu_sub(v, false); 2 },
            0x97 => { self.alu_sub(self.registers.a, false); 1 },
            0x98 => { self.alu_sub(self.registers.b, true); 1 },
            0x99 => { self.alu_sub(self.registers.c, true); 1 },
            0x9A => { self.alu_sub(self.registers.d, true); 1 },
            0x9B => { self.alu_sub(self.registers.e, true); 1 },
            0x9C => { self.alu_sub(self.registers.h, true); 1 },
            0x9D => { self.alu_sub(self.registers.l, true); 1 },
            0x9E => { let v = self.memory.rb(self.registers.hl()); self.alu_sub(v, true); 2 },
            0x9F => { self.alu_sub(self.registers.a, true); 1 },
            ...
            0xA6 => { let v = self.memory.rb(self.registers.hl()); self.alu_and(v); 2 },
            ...
            0xAE => { let v = self.memory.rb(self.registers.hl()); self.alu_xor(v); 2 },
            ...
            0xB6 => { let v = self.memory.rb(self.registers.hl()); self.alu_or(v); 2 },
            ...
            0xBE => { let v = self.memory.rb(self.registers.hl()); self.alu_cp(v); 2 },
            0xBF => { self.alu_cp(self.registers.a); 1 },
            0xC0 => { if !self.registers.getflag(Z) { self.registers.pc = self.popstack(); 5 } else { 2 } },
            0xC1 => { let v = self.popstack(); self.registers.setbc(v); 3 },
            0xC2 => { if !self.registers.getflag(Z) { self.registers.pc = self.get_word(); 4 } else { self.registers.pc += 2; 3 } },
            0xC3 => { self.registers.pc = self.get_word(); 4 },
            0xC4 => { if !self.registers.getflag(Z) { self.pushstack(self.registers.pc + 2); self.registers.pc = self.get_word(); 6 } else { self.registers.pc += 2; 3 } },
            0xC5 => { self.pushstack(self.registers.bc()); 4 },
            0xC6 => { let v = self.get_byte(); self.alu_add(v, false); 2 },
            0xC7 => { self.pushstack(self.registers.pc); self.registers.pc = 0x00; 4 },
            0xC8 => { if self.registers.getflag(Z) { self.registers.pc = self.popstack(); 5 } else { 2 } },
            0xC9 => { self.registers.pc = self.popstack(); 4 },
            0xCA => { if self.registers.getflag(Z) { self.registers.pc = self.get_word(); 4 } else { self.registers.pc += 2; 3 } },
            0xCB => { self.call_cb() },
            0xCC => { if self.registers.getflag(Z) { self.pushstack(self.registers.pc + 2); self.registers.pc = self.get_word(); 6 } else { self.registers.pc += 2; 3 } },
            0xCD => { self.pushstack(self.registers.pc + 2); self.registers.pc = self.get_word(); 6 },
            0xCE => { let v = self.get_byte(); self.alu_add(v, true); 2 },
            0xCF => { self.pushstack(self.registers.pc); self.registers.pc = 0x08; 4 },
            0xD0 => { if !self.registers.getflag(C) { self.registers.pc = self.popstack(); 5 } else { 2 } },
            0xD1 => { let v = self.popstack(); self.registers.setde(v); 3 },
            0xD2 => { if !self.registers.getflag(C) { self.registers.pc = self.get_word(); 4 } else { self.registers.pc += 2; 3 } },
            0xD4 => { if !self.registers.getflag(C) { self.pushstack(self.registers.pc + 2); self.registers.pc = self.get_word(); 6 } else { self.registers.pc += 2; 3 } },
            0xD5 => { self.pushstack(self.registers.de()); 4 },
            0xD6 => { let v = self.get_byte(); self.alu_sub(v, false); 2 },
            0xD7 => { self.pushstack(self.registers.pc); self.registers.pc = 0x10; 4 },
            0xD8 => { if self.registers.getflag(C) { self.registers.pc = self.popstack(); 5 } else { 2 } },
            0xD9 => { self.registers.pc = self.popstack(); self.setei = 1; 4 },
            0xDA => { if self.registers.getflag(C) { self.registers.pc = self.get_word(); 4 } else { self.registers.pc += 2; 3 } },
            0xDC => { if self.registers.getflag(C) { self.pushstack(self.registers.pc + 2); self.registers.pc = self.get_word(); 6 } else { self.registers.pc += 2; 3 } },
            0xDE => { let v = self.get_byte(); self.alu_sub(v, true); 2 },
            0xDF => { self.pushstack(self.registers.pc); self.registers.pc = 0x18; 4 },
            0xE0 => { let a = 0xFF00 | self.get_byte() as u16; self.memory.wb(a, self.registers.a); 3 },
            0xE1 => { let v = self.popstack(); self.registers.sethl(v); 3 },
            0xE2 => { self.memory.wb(0xFF00 | self.registers.c as u16, self.registers.a); 2 },
            0xE5 => { self.pushstack(self.registers.hl()); 4 },
            0xE6 => { let v = self.get_byte(); self.alu_and(v); 2 },
            0xE7 => { self.pushstack(self.registers.pc); self.registers.pc = 0x20; 4 },
            0xE8 => { self.registers.sp = self.alu_add16imm(self.registers.sp); 4 },
            0xE9 => { self.registers.pc = self.registers.hl(); 1 },
            0xEA => { let a = self.get_word(); self.memory.wb(a, self.registers.a); 4 },
            0xEE => { let v = self.get_byte(); self.alu_xor(v); 2 },
            0xEF => { self.pushstack(self.registers.pc); self.registers.pc = 0x28; 4 },
            0xF0 => { let a = 0xFF00 | self.get_byte() as u16; self.registers.a = self.memory.rb(a); 3 },
            0xF1 => { let v = self.popstack() & 0xFFF0; self.registers.setaf(v); 3 },
            0xF2 => { self.registers.a = self.memory.rb(0xFF00 | self.registers.c as u16); 2 },
            0xF3 => { self.setdi = 2; 1 },
            0xF5 => { self.pushstack(self.registers.af()); 4 },
            0xF6 => { let v = self.get_byte(); self.alu_or(v); 2 },
            0xF7 => { self.pushstack(self.registers.pc); self.registers.pc = 0x30; 4 },
            0xF8 => { let r = self.alu_add16imm(self.registers.sp); self.registers.sethl(r); 3 },
            0xF9 => { self.registers.sp = self.registers.hl(); 2 },
            0xFA => { let a = self.get_word(); self.registers.a = self.memory.rb(a); 4 },
            0xFB => { self.setei = 2; 1 },
            0xFE => { let v = self.get_byte(); self.alu_cp(v); 2 },
            0xFF => { self.pushstack(self.registers.pc); self.registers.pc = 0x38; 4 },
    ...
    fn call_cb(&mut self) -> u32 {
        ...
        match opcode {
            ...
            0x06 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_rlc(v); self.memory.wb(a, v2); 4 },
            ...
            0x0E => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_rrc(v); self.memory.wb(a, v2); 4 },
            ...
            0x16 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_rl(v); self.memory.wb(a, v2); 4 },
            ...
            0x1E => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_rr(v); self.memory.wb(a, v2); 4 },
            ...
            0x26 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_sla(v); self.memory.wb(a, v2); 4 },
            ...
            0x2E => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_sra(v); self.memory.wb(a, v2); 4 },
            ...
            0x36 => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_swap(v); self.memory.wb(a, v2); 4 },
            ...
            0x3E => { let a = self.registers.hl(); let v = self.memory.rb(a); let v2 = self.alu_srl(v); self.memory.wb(a, v2); 4 },
            ...
            0x46 => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 0); 3 },
            ...
            0x4E => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 1); 3 },
            ...
            0x56 => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 2); 3 },
            ...
            0x5E => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 3); 3 },
            ...
            0x66 => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 4); 3 },
            ...
            0x6E => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 5); 3 },
            ...
            0x76 => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 6); 3 },
            ...
            0x7E => { let v = self.memory.rb(self.registers.hl()); self.alu_bit(v, 7); 3 },
            ...
            0x86 => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 0); self.memory.wb(a, v); 4 },
            ...
            0x8E => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 1); self.memory.wb(a, v); 4 },
            ...
            0x96 => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 2); self.memory.wb(a, v); 4 },
            ...
            0x9E => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 3); self.memory.wb(a, v); 4 },
            ...
            0xA6 => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 4); self.memory.wb(a, v); 4 },
            ...
            0xAE => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 5); self.memory.wb(a, v); 4 },
            ...
            0xB6 => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 6); self.memory.wb(a, v); 4 },
            ...
            0xBE => { let a = self.registers.hl(); let v = self.memory.rb(a) & !(1 << 7); self.memory.wb(a, v); 4 },
            ...
            0xC6 => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 0); self.memory.wb(a, v); 4 },
            ...
            0xCE => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 1); self.memory.wb(a, v); 4 },
            ...
            0xD6 => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 2); self.memory.wb(a, v); 4 },
            ...
            0xDE => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 3); self.memory.wb(a, v); 4 },
            ...
            0xE6 => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 4); self.memory.wb(a, v); 4 },
            ...
            0xEE => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 5); self.memory.wb(a, v); 4 },
            ...
            0xF6 => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 6); self.memory.wb(a, v); 4 },
            ...
            0xFE => { let a = self.registers.hl(); let v = self.memory.rb(a) | (1 << 7); self.memory.wb(a, v); 4 },
            0xFF => { self.registers.a = self.registers.a | (1 << 7); 2 },
        }
    }

    fn alu_add(&mut self, b: u8, usec: bool) {
        let c = if usec && self.registers.getflag(C) { 1 } else { 0 };
    ...
    fn alu_daa(&mut self) {
        ...
        let mut adjust = if self.registers.getflag(C) { 0x60 } else { 0x00 };
        if self.registers.getflag(H) { adjust |= 0x06; };
